  ...
  gz::sim::EntityComponentManager & _ecm, gz::sim::EventManager & _eventManager)
{
  ...
  // i added these
  gzmsg << "AAAAAAAAA -- " << this->dataPtr->image_topic << std::endl;
  gzmsg << "AAAAAAAAA -- " << this->dataPtr->depth_image_topic << std::endl;
  gzmsg << "AAAAAAAAA -- " << this->dataPtr->simulated_image_topic << std::endl;
  ...
  // gzerr << this->dataPtr->simulated_image_topic << std::endl;
}
...
void UnderwaterCamera::CameraCallback(const gz::msgs::Image & msg)
{
  ...
  else
  {
    ...
    else
    {
	  ...
	  ros_image.header.frame_id = "high_level_robosub/block_link/underwater_camera";
      ros_image.height = msg.height();
      ros_image.width = msg.width();
      ros_image.encoding = "rgb8"; //"bgr8";
	  ...
	  size_t num_pixels = ros_image.width * ros_image.height;

	  // Convert each pixel from BGR to RGB
	  for (size_t i = 0; i < num_pixels; ++i)
	  {
	  	// Each pixel consists of 3 bytes (B, G, R for bgr8)
		size_t idx = i * 3; // Index for the start of the pixel

		// Swap the Blue and Red channels
		uint8_t b = ros_image.data[idx];      // Blue
		uint8_t g = ros_image.data[idx + 1];  // Green
		uint8_t r = ros_image.data[idx + 2];  // Red

		// Set the new values for RGB
		ros_image.data[idx] = r;              // Red goes to Blue position
		ros_image.data[idx + 1] = g;          // Green stays the same
		ros_image.data[idx + 2] = b;          // Blue goes to Red position
	  }
      this->dataPtr->image_pub->publish(ros_image);

      // Store the current image for the next iteration
      this->dataPtr->lastImage = msg;
	  gzmsg << "dave_gz_sensor_plugins::UnderwaterCamera:: PUBLISHED UNDERWATER STUFF" << std::endl;
    }
  }
}
